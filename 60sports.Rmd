...
adv_sports %>% 
   ggplot()+aes(x=SPORT,y=TOTAL,color=SPORT)+ geom_point()+labs(title = 'Degree of difficulty Adventure Sports',x='Sports',y='Total')+scale_x_discrete(guide = guide_axis(n.dodge = 2))+geom_text(aes(label=TOTAL), hjust=-0.2,size=3.3, position = position_dodge(width=1),inherit.aes = TRUE)+ theme_bw()+ geom_vline(xintercept = 1:11,color='grey')
...
#+geom_text(aes(label=TOTAL), hjust=-0.1,size=2.1, position = position_dodge(width=1),inherit.aes = TRUE)+ theme_bw()
